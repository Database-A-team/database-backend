...
export class SeatsService {
  ...
  async createSeatType(
    createSeatTypeInput: CreateSeatTypeInput,
  ): Promise<SeatType> {
  ...
  async createSeat(createSeatInput: CreateSeatInput): Promise<Seat> {
    ...
    if (createSeatInput.seatTypeId) {
      const seatType = await this.seatTypeRepository.findOne({
        id: createSeatInput.seatTypeId,
      });
      if (!seatType)
        throw new NotFoundException(
          `SeatType id ${createSeatInput.seatTypeId} is not found`,
        );
  ...
  async createSeatRow(
    createSeatRowInput: CreateSeatRowInput,
  ): Promise<SeatRow> {
    ...
    if (createSeatRowInput.screenId) {
      const screen = await getRepository(Screen).findOne({
        id: createSeatRowInput.screenId,
      });
      if (!screen)
        throw new NotFoundException(
  ...
  async updateSeatType(
    id: number,
    updateSeatTypeInput: UpdateSeatTypeInput,
  ): Promise<SeatType> {
  ...
  async updateSeatRow(id: number, updateSeatRowInput: UpdateSeatRowInput) {
    ...
    if (updateSeatRowInput.screenId) {
      let screen = await getRepository(Screen).findOne({
        id: updateSeatRowInput.screenId,
      });
      if (!screen)
        throw new NotFoundException(
          `Screen id ${updateSeatRowInput.screenId} is not found`,
        );
